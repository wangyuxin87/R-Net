...
import math
from SI_branch import AttentionModule
...
class SPM(nn.Module):
	def __init__(self, in_channels, out_channels, norm_layer):
		...
		self.conv5a = nn.Sequential(nn.Conv2d(in_channels, inter_channels, 3, padding=1, bias=False),
									...
									nn.ReLU())
		self.sa = SD_Module(inter_channels)
		self.sc1 = AttentionModule(out_channels, out_channels)  # tbq add
		self.sc2 = AttentionModule(out_channels, out_channels)  # tbq add
		self.conv51 = nn.Sequential(nn.Conv2d(inter_channels, out_channels, 3, padding=1, bias=False),
									norm_layer(out_channels),
									nn.ReLU())
		self.conv8 = nn.Sequential( nn.Conv2d(512, 512, 1), norm_layer(512),
								   nn.ReLU())
	def forward(self, x):
		feat1 = self.conv5a(x)
...
class merge(nn.Module):
	def __init__(self):
		...
		self.attention_module1 = AttentionModule(512, 512)  # tbq add
		self.attention_module2 = AttentionModule(512, 512)  # tbq add
